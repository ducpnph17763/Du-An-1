...
public class NhanVienDAO extends BarberDAO<NhanVien, Object> {
    ...
    String INSERT_SQL = "INSERT INTO NhanVien(Id_TK,HoTen,GioiTinh,DiaChi,Email,SoDienThoai,VaiTro)"
            + "values(?,?,?,?,?,?,?)";
    String UPDATE_SQL = "UPDATE NhanVien set HoTen=?,GioiTinh=?,DiaChi=?,Email=?,SoDienThoai=?,VaiTro=?,TrangThai=? where Id=?";
...
//    }
    ...
    public void deleteTT(String sdt) {
        try {
            JDBCHelper.update(DELETE_SQL, sdt);
        } catch (SQLException ex) {
            Logger.getLogger(NhanVienDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
